...
RIOSession::RIOSession(const std::shared_ptr<Core::Socket>& pSocket,
...
{
    if (m_RQ == RIO_INVALID_RQ) return false;
    RequestRecv();
    return true;
}
...
void RIOSession::SendMessage(const uint16_t packetId, const google::protobuf::Message& rfMessage)
{
    ...
    if (!m_pSendBuffer->AllocateWrite(totalSize, buffers)) return;
...
{
    bool expected = false;
    if (!m_bSendInProgress.compare_exchange_strong(expected, true)) return;
    std::vector<std::span<const std::byte>> readBuffers;
    size_t bytesToSend = m_pSendBuffer->GetReadBuffers(readBuffers);
    if (bytesToSend == 0) { m_bSendInProgress = false; return; }
    RIO_BUF buf;
    ...
    if (!Core::RioExtension::GetTable().RIOSend(m_RQ, &buf, 1, 0, &m_SendContext)) m_bSendInProgress = false;
}
...
void RIOSession::OnRioIOCompleted(bool bSuccess, DWORD bytesTransferred, Core::RioOperationType opType)
{
    ...
    if (opType == Core::RioOperationType::Receive)
    {
        m_pReceiveBuffer->CommitWrite(bytesTransferred);
        ReadReceivedBuffers();
        RequestRecv();
    }
    else if (opType == Core::RioOperationType::Send)
    {
...
void RIOSession::ReadReceivedBuffers()
{
    while (true)
    {
        ...
        if (frame.Result != Core::PacketFrameResult::Ok) break;
        if (frame.PacketOpt.has_value()) OnPacketReceived(*frame.PacketOpt);
    }
}
